mdTransform <- function(data, lambda = seq(-3,3,0.01), plot = TRUE, alpha = 0.05, verbose = TRUE){
  ...
  store2 <- lapply(1:length(lambda), function(x) if (store1[[x]] != 0) sign(data)*((abs(data)+1)^store1[[x]]-1)/store1[[x]] else sign(data)*log(abs(data)+1))
  store3 <- lapply(1:length(lambda), function(x) shapiro.test(store2[[x]])$statistic)
  
  pred.lamb<-store1[[which.max(store3)]]
  method.name<-"Estimating Modulus transformation parameter via Shapiro-Wilk test statistic"
  ...
  if (pred.lamb!=0) data.transformed<-sign(data)*((abs(data)+1)^pred.lamb-1)/pred.lamb
  if (pred.lamb==0) data.transformed<-sign(data)*log(abs(data)+1)
