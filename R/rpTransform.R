rpTransform <- function(data, plot = TRUE, alpha = 0.05, verbose = TRUE){
  ...
  if (sum(data == 0)>0) stop("Data must include nan-zero values.")
  

  method.name<-"Reciprocal transformation"
  
  
  
  data.transformed <- 1/data
