ssTransform <- function(data, lambda = seq(-3,3,0.01), plot = TRUE, alpha = 0.05, verbose = TRUE){
  ...
  store2 <- lapply(1:length(lambda), function(x) sqrt(data+store1[[x]]))
  store3 <- lapply(1:length(lambda), function(x) shapiro.test(store2[[x]])$statistic)
  
  pred.lamb<-store1[[which.max(store3)]]
  method.name<-"Estimating Square Root transformation parameter via Shapiro-Wilk test statistic"
  ...
  data.transformed<-sqrt(data+pred.lamb)
